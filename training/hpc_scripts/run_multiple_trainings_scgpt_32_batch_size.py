...
for i in range(start_run, end_run + 1):
    ...
    if 'finetune_integration_adjusted_32_batch_size' in sys.modules:
        # Force python to rerun the script by reloading the module
        importlib.reload(finetune_integration_adjusted_32_batch_size)
    else:
        # The first import executes the script
        import finetune_integration_adjusted_32_batch_size
    
    total_script_time_min = (time.time() - script_start) / 60

    # Access the global variable of the training script
    df_run = finetune_integration_adjusted_32_batch_size.robustness_results.copy()
    ...
    epoch_times = finetune_integration_adjusted_32_batch_size.epoch_times
    ...
    df_run.to_csv(f'results/scgpt_32_batch_size/result_{i}.csv', index=True)
...
if current_count < num_runs:
    ...
    for i in range(needed_samples):
        file_path = f"results/scgpt_32_batch_size/result_{i}.csv"
...
final_df.to_csv('results/scgpt_32_batch_size/combined_result.csv', index=True)
