...
for i in range(start_run, end_run + 1):
    ...
    df_run.to_csv(f'results/conditional_autoencoder_lr/result_{i}.csv', index=True)
...
if current_count < num_runs:
    ...
    for i in range(needed_samples):
        file_path = f"results/conditional_autoencoder_lr/result_{i}.csv"
...
final_df.to_csv('results/conditional_autoencoder_lr/combined_result.csv', index=True)
