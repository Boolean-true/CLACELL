...
for i in range(num_runs):
    ...
    if 'train_randomforest_bayes_scaled' in sys.modules:
        # Force python to rerun the script by reloading the module
        importlib.reload(train_randomforest_bayes_scaled)
    else:
        # The first import executes the script
        import train_randomforest_bayes_scaled
    
    total_script_time_min = (time.time() - script_start) / 60

    # Access the global variable of the training script
    df_run = train_randomforest_bayes_scaled.robustness_results.copy()
    ...
    mean_fit_per_fold = train_randomforest_bayes_scaled.opt.cv_results_['mean_fit_time']
    mean_score_per_fold = train_randomforest_bayes_scaled.opt.cv_results_['mean_score_time']
    ...
    df_run.to_csv(f'results/randomforest_scaled/result_{i}.csv', index=True)
...
final_df.to_csv('results/randomforest_scaled/combined_result.csv', index=True)
