...
for i in range(start_run, end_run + 1):
    ...
    if 'train_logisticregression_bayes' in sys.modules:
        # Force python to rerun the script by reloading the module
        importlib.reload(train_logisticregression_bayes)
    else:
        # The first import executes the script
        import train_logisticregression_bayes
    
    total_script_time_min = (time.time() - script_start) / 60

    # Access the global variable of the training script
    df_run = train_logisticregression_bayes.robustness_results.copy()
    ...
    mean_fit_per_fold = train_logisticregression_bayes.opt.cv_results_['mean_fit_time']
    mean_score_per_fold = train_logisticregression_bayes.opt.cv_results_['mean_score_time']
    ...
    df_run.to_csv(f'results/logisticregression/result_{i}.csv', index=True)
...
if current_count < num_runs:
    ...
    for i in range(needed_samples):
        file_path = f"results/logisticregression/result_{i}.csv"
...
final_df.to_csv('results/logisticregression/combined_result.csv', index=True)
