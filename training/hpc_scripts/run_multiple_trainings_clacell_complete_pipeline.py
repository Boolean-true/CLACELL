...
for i in range(start_run, end_run + 1):
    ...
    df_run.to_csv(f'results/clacell_complete_pipeline/result_{i}.csv', index=True)
...
if current_count < num_runs:
    ...
    for i in range(needed_samples):
        file_path = f"results/clacell_complete_pipeline/result_{i}.csv"
...
final_df.to_csv('results/clacell_complete_pipeline/combined_result.csv', index=True)
