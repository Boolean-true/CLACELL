...
for i in range(start_run, end_run + 1):
    ...
    mean_fit_per_fold = train_celltypist_bayessearch.opt.cv_results_['mean_fit_time']
    mean_score_per_fold = train_celltypist_bayessearch.opt.cv_results_['mean_score_time']
    ...
    df_run.to_csv(f'results/celltypist_current_python/result_{i}.csv', index=True)
...
if current_count < num_runs:
    ...
    for i in range(needed_samples):
        file_path = f"results/celltypist_current_python/result_{i}.csv"
...
final_df.to_csv('results/celltypist_current_python/combined_result.csv', index=True)
