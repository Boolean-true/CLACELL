...
for i in range(start_run, end_run + 1):
    ...
    mean_fit_per_fold = train_autoencoder_linsvc_v2.tuned_classifier.cv_results_['mean_fit_time']
    mean_score_per_fold = train_autoencoder_linsvc_v2.tuned_classifier.cv_results_['mean_score_time']
    ...
    ## Number of Epochs
    epochs = train_autoencoder_linsvc_v2.epoch + 1
    dist = "All"
    cat = "Technical_Metrics"
    ...
    df_run.to_csv(f'results/autoencoder_linsvc_v3/result_{i}.csv', index=True)
...
if current_count < num_runs:
    ...
    for i in range(needed_samples):
        file_path = f"results/autoencoder_linsvc_v3/result_{i}.csv"
...
final_df.to_csv('results/autoencoder_linsvc_v3/combined_result.csv', index=True)
