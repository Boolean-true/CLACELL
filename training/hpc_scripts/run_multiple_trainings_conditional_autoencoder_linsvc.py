...
for i in range(start_run, end_run + 1):
    ...
    if 'train_conditional_autoencoder_linsvc_v2' in sys.modules:
        # Force python to rerun the script by reloading the module
        importlib.reload(train_conditional_autoencoder_linsvc_v2)
    else:
        # The first import executes the script
        import train_conditional_autoencoder_linsvc_v2
    
    total_script_time_min = (time.time() - script_start) / 60

    # Access the global variable of the training script
    df_run = train_conditional_autoencoder_linsvc_v2.robustness_results.copy()
    ...
    mean_fit_per_fold = train_conditional_autoencoder_linsvc_v2.tuned_classifier.cv_results_['mean_fit_time']
    mean_score_per_fold = train_conditional_autoencoder_linsvc_v2.tuned_classifier.cv_results_['mean_score_time']
    ...
    ## Number of Epochs
    epochs = train_conditional_autoencoder_linsvc_v2.epoch + 1
    dist = "All"
    cat = "Technical_Metrics"
    ...
    df_run.to_csv(f'results/conditional_autoencoder_linsvc_v3/result_{i}.csv', index=True)
...
if current_count < num_runs:
    ...
    for i in range(needed_samples):
        file_path = f"results/conditional_autoencoder_linsvc_v3/result_{i}.csv"
...
final_df.to_csv('results/conditional_autoencoder_linsvc_v3/combined_result.csv', index=True)
